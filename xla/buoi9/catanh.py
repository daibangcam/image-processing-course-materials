...
cropped_first = None
contour_data = []
boxes_data = []  # 👉 lưu thông tin (box, cx, cy, filename) cho bước add text
...
for contour in contours:
    ...
    contour_data.append((contour, cx))  # lưu contour cùng hoành độ tâm

# 👉 Sắp xếp danh sách theo cx (trái sang phải)
contour_data = sorted(contour_data, key=lambda x: x[1])

count = 0
cropped_first = None

# 👉 Duyệt qua các contour đã sắp xếp
for contour, cx in contour_data:
    rect = cv2.minAreaRect(contour)
    ...
    # --- Lưu toạ độ box và tên file để add text sau ---
    boxes_data.append({
        "box": box,
        "cx": cx,
        "cy": cy,
...
# 👉 Sau khi xử lý xong, bạn có thể add text từng biển bằng boxes_data
print("📦 Đã lưu thông tin box:")
for b in boxes_data:
    print(f" - {b['filename']} | Tâm ({b['cx']:.1f}, {b['cy']:.1f})")


print(f"\n🔹 Tổng số biển số hợp lệ đã cắt: {count}")
cv2.imshow("SELECT BIEN", output)
pytesseract.pytesseract.tesseract_cmd = r"C:\\Program Files\\Tesseract-OCR\\tesseract.exe"
# === CHẠY TOÀN BỘ ẢNH TRONG THƯ MỤC ===
folder_path = "output_regions"  # thư mục chứa ảnh
image_files = [f for f in os.listdir(folder_path)
               if f.lower().endswith(('.png', '.jpg', '.jpeg'))]

print(f"🔍 Tìm thấy {len(image_files)} ảnh trong '{folder_path}'")
...
for file_name in image_files:
    ...
    if img is None:
        print(f"⚠ Không đọc được ảnh: {file_name}")
        continue

    # ======= (phần code OCR của bạn bên dưới giữ nguyên) =======
    ...
    if len(boxes_data) > 0:
        first_box_data = boxes_data[i]
        box = first_box_data["box"]



        # --- Tính vị trí đặt chữ ---
        x_text = int(np.mean(box[:, 0]))  # trung bình hoành độ
        y_text = int(np.min(box[:, 1])) - 10  # ngay phía trên box

        # --- In text lên ảnh ---
        cv2.putText(output, fixed_text, (x_text - 60, y_text),
